...
def summarize_diagnostics(history, since_step=10):
    ...
    ax2.plot(
        history.history["localization_point_accuracy_radius_8.75"][since_step:],
        color="blue",
        label="train",
    )
    ax2.plot(
        history.history["val_localization_point_accuracy_radius_8.75"][since_step:],
        color="orange",
        label="test",
    )
    ...
    ax2.plot(
        history.history["localization_point_accuracy_radius_4.375"][since_step:],
        color="blue",
        label="train",
    )
    ax2.plot(
        history.history["val_localization_point_accuracy_radius_4.375"][since_step:],
        color="orange",
        label="test",
    )
...
class MyModel(keras.Model):
    ...
    def __init__(self, **kwargs):
        ...
        self.dense_1 = keras.layers.Dense(
            256, activation="relu", kernel_regularizer=tf.keras.regularizers.l2()
        )
        ...
        self.dense_4 = keras.layers.Dense(
            64, activation="relu", kernel_regularizer=tf.keras.regularizers.l2()
        )
...
def create_my_model():
    ...
    model.compile(
        ...
        metrics=[
            LocalizationPointAccuracy(accuracy=True, radius=8.75),
...
def main():
    ...
    weights_path_check_loss = "weights/model_weights_landmark_{0}_check_loss".format(
        r_n
    )
    ...
    for weights in [weights_path_check, weights_path_check_loss]:
        ...
        print(
            (
                "loss: {0} - localization_point_accuracy_radius_8.75: {1} - localization_outside_radius_8.75_distance: {2} - "
                + "localization_point_accuracy_radius_4.375: {3} - localization_outside_radius_4.375_distance: {4} - "
                + "localization_outside_radius_0_distance: {5}"
            ).format(*val_res[0])
        )
        print("*" * 60)
