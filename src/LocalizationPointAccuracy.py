...
class LocalizationPointAccuracy(keras.metrics.Metric):
    ...
    LANDMARK_OUTSIDE_AVERAGE_DISTANCE_FORMAT = (
        "localization_outside_radius_{0}_distance"
    )

    def __init__(self, name=None, accuracy=True, radius=5, **kwargs):
        if name == None:
            name = (
                self.LANDMARK_ACCURACY_FORMAT.format(radius)
                if accuracy
                else self.LANDMARK_OUTSIDE_AVERAGE_DISTANCE_FORMAT.format(radius)
            )
        ...
        self.total_outside_distance = self.add_weight(
            name="total_outside_distance", initializer="zeros"
        )
    ...
    def update_state(self, y_true, y_pred, sample_weight=None):
        ...
        if len(tf.shape(diffs)) == 1:
            diffs = tf.reduce_sum(
                tf.reshape(diffs, (1, tf.cast(len(diffs) / 2, tf.int32), 2)), axis=2
            )
        else:
            diffs = tf.reduce_sum(
                tf.reshape(
                    diffs,
                    (tf.shape(diffs)[0], tf.cast(tf.shape(diffs)[1] / 2, tf.int32), 2),
                ),
                axis=2,
            )
        diffs = tf.sqrt(diffs)
        ...
        self.point_outside.assign_add(
            tf.reduce_sum(tf.cast(outside_points, tf.float32))
        )
