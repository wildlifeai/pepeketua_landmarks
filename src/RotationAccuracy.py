...
class RotationAccuracy(keras.metrics.Metric):
    ...
    def __init__(self, name=None, accuracy=True, angle_radius=0, **kwargs):
        if name == None:
            name = (
                self.ROTATION_ACCURACY_FORMAT.format(angle_radius)
                if accuracy
                else self.ROTATION_OUTSIDE_AVERAGE_DISTANCE_FORMAT.format(angle_radius)
            )
        ...
        self.total_angle_distance_outside = self.add_weight(
            name="total_angle_distance_outside", initializer="zeros"
        )
        self.num_points_outside = self.add_weight(
            name="num_points_outside", initializer="zeros"
        )
    ...
    def update_state(self, y_true, y_pred, sample_weight=None):
        ...
        self.num_points_outside.assign_add(
